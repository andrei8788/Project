...
export class VerificationOfEnteredData {
  ...
  constructor() {
    this.guesses = 0; // number of whistles
    this.boardSize = 7;
  ...
  parseGuess(guess) { // checks, converts, and passes data to the processGuess property
    ...
    if (guess === null || guess.length !== 2) {
      ...
      const row = alphaBet.indexOf(firstChar); // convert the letter to a number (the sequence number in the array)
